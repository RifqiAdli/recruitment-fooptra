...
import { LayoutDashboard, Briefcase, FileText, Users, Calendar, Megaphone, HelpCircle, Settings, Menu, ArrowLeft } from 'lucide-react';
          ...
          {sidebarLinks.map((link) => (
            <Link key={link.href} to={link.href} onClick={() => setSidebarOpen(false)}
              className={cn('flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors',
                location.pathname === link.href ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:text-foreground hover:bg-accent')}>
